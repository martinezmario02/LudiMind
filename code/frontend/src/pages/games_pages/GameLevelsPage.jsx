...
export default function LevelsPage() {
  ...
  const lastCompleted = levels.reduce((max, lvl) => {
    if (lvl.score === 3 && lvl.level_number > max) {
      return lvl.level_number;
    }
    return max;
  }, 0);
      ...
      <div className="w-full max-w-3xl min-w-[400px] mx-auto my-8 shadow-lg rounded-xl overflow-hidden bg-white">
        ...
        <div className="bg-gray-100 w-full py-6 px-6">
          ...
          <p className="text-md text-gray-600">
            ...
            <span className="font-semibold">
              {categoryTranslations[game.category]}
            </span>
          </p>
        </div>
        ...
        <div className="w-full py-6 px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {levels.map((level) => {
              const isPlayed = level.score > 0;
              const isUnlocked =
                isPlayed || level.level_number <= lastCompleted + 1;
